...
export interface IMarketDataProvider {
  ...
  searchInstruments(query: string): Promise<MarketQuote[]>;
  getMutualFunds(): Promise<MutualFundData[]>;
}

// Authentic Base Universe of Indian Equities and Indices
export const INDIAN_INSTRUMENT_UNIVERSE: {
  quote: MarketQuote;
...
export const AUTHENTIC_MUTUAL_FUNDS: MutualFundData[] = [
  {
...
export class LicensedMarketDataProvider implements IMarketDataProvider {
  ...
  async getQuotes(symbols: string[]): Promise<MarketQuote[]> {
    ...
    const matching = INDIAN_INSTRUMENT_UNIVERSE.filter((item) =>
      symbolSet.has(item.quote.symbol.toUpperCase())
    ).map((item) => this.applyMicroTick(item.quote));

    return matching;
  }

  async getQuote(symbol: string): Promise<MarketQuote | null> {
    const match = INDIAN_INSTRUMENT_UNIVERSE.find(
      (item) => item.quote.symbol.toUpperCase() === symbol.toUpperCase()
    );
  ...
  async getMarketOverview(): Promise<{
    ...
    const allQuotes = INDIAN_INSTRUMENT_UNIVERSE.map((item) => this.applyMicroTick(item.quote));
  ...
  async getCompanyFundamentals(symbol: string): Promise<CompanyFundamentals | null> {
    const item = INDIAN_INSTRUMENT_UNIVERSE.find(
      (entry) => entry.quote.symbol.toUpperCase() === symbol.toUpperCase()
    );
  ...
  async searchInstruments(query: string): Promise<MarketQuote[]> {
    ...
    if (!q) {
      return INDIAN_INSTRUMENT_UNIVERSE.map((i) => this.applyMicroTick(i.quote)).slice(0, 10);
    }

    return INDIAN_INSTRUMENT_UNIVERSE.filter(
      (entry) =>
        entry.quote.symbol.toUpperCase().includes(q) ||
        entry.quote.name.toUpperCase().includes(q) ||
        entry.quote.sector.toUpperCase().includes(q)
    ).map((entry) => this.applyMicroTick(entry.quote));
  }

  async getMutualFunds(): Promise<MutualFundData[]> {
    return AUTHENTIC_MUTUAL_FUNDS;
  }

  // Micro-tick smoother ensuring prices fluctuate naturally with authentic decimals
  private applyMicroTick(quote: MarketQuote): MarketQuote {
    const now = new Date();
