...
function toAuthError(err: unknown): AuthError {
  ...
  switch (raw) {
    ...
    default:
      ...
      // Firebase reports a disabled Phone provider (or a disabled Identity
      // Toolkit API) as an opaque internal error carrying this token. Naming it
      // turns an unactionable "try again" into something a developer can fix.
      // 18002: Play Integrity could not vouch for this build. Sideloaded and
      // debug-signed APKs are not recognised by Play, so app verification fails
      // before any SMS is attempted.
      if (/18002|PlayIntegrity|not Recognized by Play Store/i.test(nativeMessage)) {
        return new AuthError(
          'not-configured',
          'This build cannot be verified by Google Play, so an SMS cannot be sent. Use a Firebase test number, or install a Play-distributed build.'
        );
      }
      // 17006: the destination country is not in the project's SMS region
      // allowlist. Without naming it, this is indistinguishable from a bad
      // number to whoever is testing.
      if (/17006|region enabled by the app developer/i.test(nativeMessage)) {
        return new AuthError(
          'operation-not-allowed',
          'SMS is not enabled for this region on the Firebase project. Allow the country under Authentication settings, then try again.'
        );
      }
      if (/CONFIGURATION_NOT_FOUND|17499/i.test(nativeMessage)) {
        return new AuthError(
          'operation-not-allowed',
          'Phone sign-in is not enabled for this Firebase project yet. Enable the Phone provider in Firebase Authentication, then try again.'
        );
      }
