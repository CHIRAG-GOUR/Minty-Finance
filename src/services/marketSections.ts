...
export const MarketSections = {
  ...
  async getStocksInNews(): Promise<SectionResult<never>> {
    if (!PROVIDER_CAPABILITIES.news) {
      return unavailable(UNAVAILABLE_REASONS.news);
    }
    return unavailable(UNAVAILABLE_REASONS.news);
  },

  async getMostBoughtETFs(): Promise<SectionResult<MoverRow>> {
    if (!PROVIDER_CAPABILITIES.etfUniverse) {
      return unavailable(UNAVAILABLE_REASONS.etf);
    }
    return unavailable(UNAVAILABLE_REASONS.etf);
  },
