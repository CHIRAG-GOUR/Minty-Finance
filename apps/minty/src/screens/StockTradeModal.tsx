...
const StockTradeModalBody: React.FC<StockTradeModalProps> = ({ visible, data, onClose }) => {
  ...
  const handleExecute = async (action: 'buy' | 'sell') => {
    ...
    if (shares <= 0) {
      showToast('Invalid Quantity', 'Please enter at least 1 share.', 'warning');
      return;
    }
    if (action === 'buy' && netTotal > wallet.cashBalance) {
      showToast(
        'Insufficient Cash',
        'Your cash balance is lower than this trade value.',
        'warning'
      );
      return;
    }
      ...
      >
        ...
        <View style={styles.topHeaderBar}>
          <View style={styles.topHeaderLeft}>
            <CompanyLogo symbol={stock.symbol} name={stock.name} logoUrl={stock.logoUrl} size={44} />
        ...
        <View style={styles.tradeControlCard}>
          <View style={styles.tradeControlHeader}>
            <Text style={styles.tradeControlTitle}>Place Order</Text>
        ...
        <View style={styles.balanceReminder}>
          ...
          <Text style={styles.balanceReminderText}>
            Available cash: {formatCurrency(wallet.cashBalance)}
          </Text>
        </View>
      </ScrollView>
