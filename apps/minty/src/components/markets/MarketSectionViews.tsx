...
// ---------------------------------------------------------------------------
// Layout F — featured stock tile (Top Stocks)
// ---------------------------------------------------------------------------

export const FeaturedStockTile: React.FC<{
  stock: StockItem;
  onPress: () => void;
}> = ({ stock, onPress }) => (
  <TouchableOpacity
    ...
    accessibilityRole="button"
    accessibilityLabel={`${stock.name}, ${formatCurrencyOrDash(stock.currentPrice, true)}, ${formatPercentage(stock.changePercent)}`}
  >
    <CompanyLogo symbol={stock.symbol} name={stock.name} logoUrl={stock.logoUrl} size={38} />
    ...
    <Text
      style={[styles.featuredChange, { color: moveColor(stock.changePercent) }]}
      numberOfLines={1}
    >
      {formatPercentage(stock.changePercent, true, 2)}
    </Text>
...
export const EtfRowView: React.FC<{
  stock: StockItem;
  category: string;
  volume: number;
  onPress: () => void;
}> = ({ stock, category, volume, onPress }) => (
  <TouchableOpacity
    onPress={onPress}
    activeOpacity={0.75}
    style={styles.etfRow}
  ...
  >
    ...
    <View style={styles.etfTextCol}>
      <Text style={styles.rankedSymbol} numberOfLines={1}>
        {stock.name}
      </Text>
      <Text style={styles.rankedName} numberOfLines={1}>
        {category} · {formatCompactNumber(volume)} traded
      </Text>
    </View>
    <View style={styles.rankedRight}>
      <Text style={styles.rankedPrice} numberOfLines={1}>
...
// ---------------------------------------------------------------------------
// Layout H — news timeline item
// ---------------------------------------------------------------------------

export const NewsRowView: React.FC<{
  headline: string;
  source: string;
  publishedAt: string;
  stock: StockItem;
  onPress: () => void;
}> = ({ headline, source, publishedAt, stock, onPress }) => (
  <TouchableOpacity
    onPress={onPress}
    activeOpacity={0.75}
  ...
  >
    <View style={styles.newsRail}>
      <CompanyLogo symbol={stock.symbol} name={stock.name} size={30} />
      <View style={styles.newsLine} />
    </View>
    <View style={styles.newsTextCol}>
      <View style={styles.newsMetaRow}>
        <Text style={styles.newsSymbol} numberOfLines={1}>
          {stock.symbol}
        </Text>
        <Text
          style={[styles.newsChange, { color: moveColor(stock.changePercent) }]}
          numberOfLines={1}
        >
          {formatPercentage(stock.changePercent, true, 2)}
        </Text>
      ...
      <Text style={styles.newsSource} numberOfLines={1}>
        {source}
        {publishedAt ? ` · ${publishedAt.slice(5, 16)}` : ''}
      </Text>
    </View>
  </TouchableOpacity>
);
...
const styles = StyleSheet.create({
  ...
  etfTextCol: { flex: 1, minWidth: 0 },
  newsRow: { flexDirection: 'row', gap: 10, paddingVertical: 10 },
  newsRail: { alignItems: 'center', width: 30, flexShrink: 0 },
  newsLine: {
    flex: 1,
    width: 2,
    marginTop: 6,
  ...
  newsTextCol: { flex: 1, minWidth: 0, gap: 2, paddingBottom: 4 },
  newsMetaRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  newsSymbol: { fontSize: 11, fontWeight: '900', color: THEME.colors.textSecondary },
  newsChange: { fontSize: 11, fontWeight: '800' },
  newsHeadline: {
    fontSize: 13,
    lineHeight: 18,
    fontWeight: '700',
  ...
  newsSource: { fontSize: 10, color: THEME.colors.textMuted, fontWeight: '600' },
  rankedRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 10,
