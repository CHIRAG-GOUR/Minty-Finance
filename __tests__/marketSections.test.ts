...
describe('sections the provider cannot supply', () => {
  it('reports unavailable instead of inventing a ranking', async () => {
    for (const load of [
      MarketSections.getMostBought(),
      MarketSections.getMostBoughtMTF(),
      MarketSections.getMostBoughtETFs(),
      MarketSections.getStocksInNews(),
    ]) {
